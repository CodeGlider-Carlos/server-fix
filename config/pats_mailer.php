...
Funciona con SMTP SSL 465 tipo:
- smtpout.secureserver.net
- usuario noreply@pasaporteatusalud.com

No depende de PHPMailer.
*/
...
if (!function_exists('pats_smtp_send_mail')) {
  function pats_smtp_send_mail(string $to, string $subject, string $html, string $text = ''): bool {
    ...
    $message = implode(pats_mailer_eol(), $headers) . pats_mailer_eol() . pats_mailer_eol();
    $message .= '--' . $boundary . pats_mailer_eol();
    $message .= 'Content-Type: text/plain; charset=UTF-8' . pats_mailer_eol();
    $message .= 'Content-Transfer-Encoding: 8bit' . pats_mailer_eol() . pats_mailer_eol();
    $message .= $text . pats_mailer_eol() . pats_mailer_eol();

    $message .= '--' . $boundary . pats_mailer_eol();
    $message .= 'Content-Type: text/html; charset=UTF-8' . pats_mailer_eol();
    $message .= 'Content-Transfer-Encoding: 8bit' . pats_mailer_eol() . pats_mailer_eol();
    $message .= $html . pats_mailer_eol() . pats_mailer_eol();
    $message .= '--' . $boundary . '--' . pats_mailer_eol();

    /*
      SMTP DATA termina con punto en línea sola.
      Si una línea inicia con punto, se escapa con punto doble.
    */
    $message = preg_replace('/^\./m', '..', $message);
    fwrite($fp, $message . pats_mailer_eol() . '.' . pats_mailer_eol());
...
<body style="margin:0;padding:0;background:#f3f7ff;font-family:Arial,Helvetica,sans-serif;color:#102a56;">
  <div style="max-width:640px;margin:0 auto;padding:28px 16px;">
    ...
    $text = "Tu Pasaporte fue registrado.\n\n"
